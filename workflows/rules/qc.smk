...
rule filter_cells:
    ...
    """
    input:
        h5ad = lambda wildcards: config['datasets'][wildcards.dataset]["input_path"]
    output:
        h5ad = "results/{dataset}/filtered.h5ad",
        qc_plots = directory("reports/{dataset}/qc_plots")
    params:
        min_genes = lambda wildcards: config['datasets'][wildcards.dataset].get("min_genes", 200),
        max_genes = lambda wildcards: config['datasets'][wildcards.dataset].get("max_genes", 6000),
        max_pct_mt = lambda wildcards: config['datasets'][wildcards.dataset].get("max_pct_mt", 15),
        min_cells_per_gene = lambda wildcards: config['datasets'][wildcards.dataset].get("min_cells_per_gene", 3)
    log:
...
rule qc_report:
    ...
    """
    input:
        raw = lambda wildcards: config['datasets'][wildcards.dataset]["input_path"],
